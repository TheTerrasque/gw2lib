...
def genItemChatCode(item_id, item_count=1, itemtype=2):
    ...
    """
    hexa = bytearray(itemtype) + bytearray(item_count)  # Item type, number
    for i in range(4):
        hexa += bytearray(item_id % 256)
        item_id //= 256
...
class Struct:
    ...
    def __repr__(self):
        spc = " " * self.__level * 2
        return '[Structure]\n' + spc + '%s' % str(
            ('\n' + spc).join('%s : %s' % (k, repr(v)) for (k, v) in list(self.__dict__.items()) if k[0] != "_"))
    ...
    def load(self, filename="webcache.p"):
        try:
            with open(filename, "rb") as f:
                ...
                self.oldest = datetime(1990, 0o1, 0o1)
        except IOError:
            pass
